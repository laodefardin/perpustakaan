...
<?php if ($_SESSION['siswa']) {?>
					<div class="form-group">
						...
						<select class="form-control" name="nama" readonly>
							<?php 
					$query = $koneksi->query("SELECT * FROM tb_anggota WHERE nis='$_SESSION[siswa]'");
  ...
  <?php  } ?>
					...
					<div class="form-group">
						...
						<input class="form-control" type="text" name="kembali" value="<?php echo $kembali; ?>"
							readonly />
					</div>
		...
		while ($hasil=$query->fetch_assoc()) {
			...
			elseif(!$num1)
			{
				...
				if ($user === 'admin'){
					...
					$qt = $koneksi->query("INSERT INTO tb_transaksi VALUES (null, '$id_buku', '$judul', '$id_siswa', '$siswa', '$tgl_pinjam', '$tgl_kembali', 'Belum diambil', null)") or die ("Gagal Masuk".mysql_error());
